    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 px-2">
        ...
        <div class="flex mt-6 items-center justify-between">
            ...
            <div class="flex gap-1">
                <form action="{{ route('admin.tenants.index') }}" method="GET" class="flex items-center gap-2">
                ...
                <x-dropdown>
                    ...
                    <x-slot name="content">
                        ...
                        <x-dropdown-link :href="route('admin.tenants.index', array_merge(request()->query(), ['kategori' => null]))"
                            class="{{ is_null(request('kategori')) ? 'bg-gray-200 font-bold' : '' }}">
                            {{ __('All Categories') }}
                        </x-dropdown-link>

                        @foreach ($allCategories as $category)
                            <x-dropdown-link :href="route('admin.tenants.index', array_merge(request()->query(), ['kategori' => $category]))"
                ...
                <x-dropdown>
                    ...
                    <x-slot name="content">
                        <x-dropdown-link :href="route('admin.tenants.index', array_merge(request()->query(), ['status' => null]))"
                            class="{{ is_null(request('status')) ? 'bg-gray-200 font-bold' : '' }}">
                            {{ __('All Status') }}
                        </x-dropdown-link>

                        <x-dropdown-link :href="route('admin.tenants.index', array_merge(request()->query(), ['status' => 'active']))"
                            class="{{ request('status') === 'active' ? 'bg-gray-200 font-bold' : '' }}">
                            {{ __('Active') }}
                        </x-dropdown-link>
                        
                        <x-dropdown-link :href="route('admin.tenants.index', array_merge(request()->query(), ['status' => 'inactive']))"
            ...
            <p class="text-gray-500 my-2">
                ...
                @if ($status)
                    <span class="badge {{ $status === 'active' ? 'bg-green-500' : 'bg-red-500' }} text-white inline-flex items-center gap-2 px-2 py-1 rounded">
                        ...
                        <a href="{{ route('admin.tenants.index', array_merge(request()->except('status'), ['page' => 1])) }}"
                ...
                @if ($kategori)
                    <span class="badge bg-blue-500 text-white inline-flex items-center gap-2 px-2 py-1 rounded">
                        ...
                        <a href="{{ route('admin.tenants.index', array_merge(request()->except('kategori'), ['page' => 1])) }}"
                           class="text-white hover:text-gray-300 ml-1">
                            &times;
                        </a>
                    </span>
                ...
                @if ($search)
                    <span class="badge bg-purple-500 text-white inline-flex items-center gap-2 px-2 py-1 rounded">
                        ...
                        <a href="{{ route('admin.tenants.index', array_merge(request()->except('search'), ['page' => 1])) }}"
                           class="text-white hover:text-gray-300 ml-1">
                            &times;
                        </a>
                    </span>
            ...
            <div x-data="{ tenant: {} }" x-on:set-tenant.window="tenant = $event.detail">
                <div class="p-6 items-center">
                    <div class="tenant-item">
                        <img :src="tenant.foto" alt="foto" class="tenant-image">
                        <div class="tenant-info">
                            <span class="tenant-category" x-text="tenant.kategori"></span>
                            <h3 class="tenant-name" x-text="tenant.nama"></h3>
                            <p class="tenant-description" x-text="tenant.deskripsi"></p>
                            <div class="tenant-details">
                                <p><i class="fas fa-map-marker-alt"></i> <span x-text="tenant.location"></span></p>
                                <p><i class="fas fa-clock"></i> <span x-text="tenant.jam_buka + ' - ' + tenant.jam_tutup"></span></p>                                
                            </div>
                        </div>
                    </div>
